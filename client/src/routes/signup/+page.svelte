<script lang="ts">
    import {ChevronRight, Cross1, EnvelopeClosed, LockClosed, Person} from "@steeze-ui/radix-icons";
...
<PrimroseView title="Sign up" headline={import.meta.env.VITE_APP_NAME}>
    {#if error != null}
    ...
    <div class="px-2">
        <div class="flex flex-row gap-4 items-center pt-4 border-b pb-2 md:max-w-sm {loadingClass}">
            ...
            <input disabled={loading} type="text" placeholder="Username" class="outline-none bg-transparent" bind:value={name}/>
        </div>
        <div class="flex flex-row gap-4 items-center pt-4 border-b pb-2 md:max-w-sm {loadingClass}">
        ...
        <div class="flex flex-row gap-1 items-center">
            <button disabled={loading} on:click={create} class="{loadingClass} p-2 pr-4 my-4 flex flex-row gap-2 items-center hover:bg-white hover:text-black animated duration-700">
